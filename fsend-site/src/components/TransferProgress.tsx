...
export function TransferProgress(props: TransferProgressProps) {
  const pct = () => {
    ...
    return Math.min(100, (props.progress.totalTransferred / props.progress.totalSize) * 100);
  };
    ...
    <div class="w-full">
      <div class="mb-6">
        <div class="flex justify-between items-center mb-2">
          ...
          <span class="text-sm text-gray-600 dark:text-gray-400">
            {formatBytes(props.progress.totalTransferred)} / {formatBytes(props.progress.totalSize)}
          </span>
        </div>
          ...
          <Show when={props.progress.speed > 0}>
            <div class="text-right">
              <div class="text-sm font-medium text-gray-700 dark:text-gray-300">
                {props.speedLabel ?? 'Speed'}: {formatSpeed(props.progress.speed)}
              </div>
      ...
      <div class="border dark:border-neutral-700 rounded-lg p-4 max-h-80 overflow-y-auto space-y-4">
        <For each={props.progress.entries}>
          {(entry) => {
            const entryPct = () =>
              entry.size === 0 ? 100 : Math.min(100, (entry.transferred / entry.size) * 100);
