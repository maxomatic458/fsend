...
export async function runFallbackReceiver(
  ...
  try {
    ...
    // from transient 'disconnected' pc state.
    ...
    // Accept/reject (races disconnect/abort)
    ...
    callbacks.onTransferring(
      recvTrees.map((t) => ({ name: t.name, size: treeSize(t), skip: 0, isDir: t.type === 'Dir' })),
    );
    ...
    if (!hasDirectories && !multipleFiles && flatFiles.length === 1) {
      ...
      const zipName = offeredFiles.length === 1 ? `${offeredFiles[0].name}.zip` : 'fsend-files.zip';
